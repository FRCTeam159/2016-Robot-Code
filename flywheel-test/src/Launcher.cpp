...
void Launcher::Aim(float range)//takes horizontal range, in meters
{
	...
	float Vv0=sqrt(pow(term, 2)*(pow(2.71828, dH*2*9.81/pow(term,2))-1));//.57... is euler's constant
	...
	std::cout<<"Velocity (horizontal) = "<<Vh0<<std::endl;
	//SetAngle(atan(Vv0/Vh0)*(180/3.1415));//calculate and set target angle
	std::cout<<"target angle = "<<atan(Vv0/Vh0)*(180/3.1415)<<"degrees"<<std::endl;
	float V0= sqrt(pow(Vh0,2)+pow(Vv0, 2));
	float r=.058;
	float Iwheel=19.42;
	float targetSpeed= V0*sqrt(pow((1/r),2)+(.295/(2*Iwheel)));//target speed in rad/s
